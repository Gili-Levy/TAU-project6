				...
				if len(rhs) == 2:  # rule like A -> XY (not like A -> a)
					...
					if X in table[i][k] and Y in table[k][j]:
						if lhs not in table[i][j]:
							...
							table[i][j][lhs] = min(table[i][j][lhs], max(1+table[i][k][X], 1+table[k][j][Y]))
...
def test():
	...
	leibniz_formula = [1, -1/3, 1/5, -1/7, 1/9, -1/11, 1 /
                    13, -1/15, 1/17, -1/19, 1/21, -1/23, 1/25, -1/27, 1/29]
	leibniz_sum_powers_of_2 = [4*leibniz_formula[0], 4*sum(
		leibniz_formula[:3]), 4*sum(leibniz_formula[:7]), 4*sum(leibniz_formula[:15])]
